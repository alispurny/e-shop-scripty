/**
 * MyBears Product Guide 2.0 — produkční samostatná verze CZ
 * Version: 2.0.0-cz
 ...
 *   labAnalysesUrl: '/laboratorni-analyzy',
...
(function () {
  ...
  const STYLE_ID = 'mbpg-complete-styles-v200-cz';
  ...
  },
  {
    ...
    "keyIngredients": [
      ...
      "keratin",
      "MSM",
      "kyselina hyaluronová 200 mg (50–100 kDa)",
      "koenzym Q10",
  ...
  const QUESTIONS = [
    {
      ...
      description: 'Nejdřív zvolte, komu má doporučení sloužit.',
      options: [
        { value: 'adult', label: 'Pro dospělého', description: 'Doplňky a rutiny určené dospělým', icon: 'user' },
        { value: 'child', label: 'Pro dítě od 3 let', description: 'Pouze produkty výslovně určené dětem', icon: 'bear' },
        { value: 'family', label: 'Pro celou rodinu', description: 'Rodinné kombinace s oddělenými produkty', icon: 'family' }
      ]
    },
    {
      id: 'goal',
      title: 'Na co se chcete zaměřit?',
      description: 'Vyberte hlavní oblast. Doporučení se bude řídit především tímto cílem.',
      options: [
        { value: 'sleep', label: 'Spánek a večerní rutina', description: 'Produkty vhodné pro večerní režim', icon: 'moon', audiences: ['adult'] },
        { value: 'calm', label: 'Klid a psychická pohoda', description: 'Produkty pro klidnější denní nebo večerní rutinu', icon: 'leaf-heart', audiences: ['adult'] },
        { value: 'focus', label: 'Soustředění a mentální výkon', description: 'Produkty pro pracovní nebo studijní rutinu', icon: 'brain', audiences: ['adult'] },
        { value: 'energy', label: 'Energie a aktivní režim', description: 'Produkty vhodné pro aktivní každodenní režim', icon: 'bolt', audiences: ['adult'] },
        { value: 'immunity', label: 'Imunita', description: 'Vitamíny, minerály a rodinné varianty', icon: 'shield', audiences: ['adult', 'child', 'family'] },
        { value: 'beauty', label: 'Vlasy, pokožka a nehty', description: 'Beauty produkty, minerály a kolagen', icon: 'sparkles', audiences: ['adult'] },
        { value: 'mature_skin', label: 'Péče o pleť 30+', description: 'Kolagen a antioxidačně zaměřené složení', icon: 'flower', audiences: ['adult'] },
        { value: 'digestion', label: 'Trávení a mikroflóra', description: 'Probiotika a související rutiny', icon: 'digestion', audiences: ['adult'] },
        { value: 'sport', label: 'Sport a fyzická výkonnost', description: 'Hořčík, pre-workout a sportovní rutiny', icon: 'dumbbell', audiences: ['adult'] },
        { value: 'joints', label: 'Klouby a pohybový aparát', description: 'Komplexní kloubní výživa a související produkty', icon: 'bone', audiences: ['adult'] },
        { value: 'daily', label: 'Každodenní základ', description: 'Univerzální vitamíny, minerály a omega-3', icon: 'sun', audiences: ['adult'] },
        { value: 'child_daily', label: 'Vitamíny pro dítě', description: 'Denní gummies pro děti od 3 let', icon: 'citrus', audiences: ['child'] }
      ]
    },
    {
      id: 'form',
      title: 'Jaká forma vám vyhovuje?',
      description: 'Pokud nemáte preferenci, nechte rozhodnout především složení a zaměření produktu.',
      options: [
        { value: 'any', label: 'Je mi to jedno', description: 'Rozhodne hlavně zaměření a složení', icon: 'check-circle' },
        { value: 'gummies', label: 'Gumoví medvídci', description: 'Pohodlná ochucená forma', icon: 'bear' },
        { value: 'capsules', label: 'Kapsle nebo softgely', description: 'Klasická forma bez ochucení', icon: 'pill', audiences: ['adult'] },
        { value: 'powder', label: 'Rozpustný prášek', description: 'Kolagenové směsi s odměrkou', icon: 'spoon', audiences: ['adult'] },
        { value: 'mixed', label: 'Kombinace více forem', description: 'Vhodné, pokud vám nevadí více produktů v rutině', icon: 'layers', audiences: ['adult', 'family'] }
      ]
    },
    {
      id: 'diet',
      title: 'Je pro vás některá vlastnost produktu důležitá?',
      description: 'Vyberte pouze tehdy, pokud je tato podmínka pro vás zásadní.',
      options: [
        { value: 'any', label: 'Bez zvláštního požadavku', description: 'Neomezovat doporučení podle těchto vlastností', icon: 'circle' },
        { value: 'vegan', label: 'Pouze veganské', description: 'Jen produkty s potvrzeným veganským provedením', icon: 'leaf' },
        { value: 'sugarFree', label: 'Pouze bez cukru', description: 'Jen produkty jednoznačně označené jako bez cukru', icon: 'sugar-off' },
        { value: 'glutenFree', label: 'Pouze bez lepku', description: 'Jen produkty s potvrzeným bezlepkovým provedením', icon: 'wheat-off' },
        { value: 'halal', label: 'Pouze halal', description: 'Jen produkty s potvrzeným halal označením', icon: 'crescent-star' }
      ]
    },
    {
      id: 'stimulants',
      title: 'Chcete se vyhnout kofeinu?',
      description: 'Tuto otázku zobrazujeme jen tehdy, když může skutečně ovlivnit výběr.',
      audiences: ['adult'],
      options: [
        { value: 'any', label: 'Nevadí mi', description: 'Mohou se zobrazit i produkty s kofeinem nebo zeleným čajem', icon: 'coffee' },
        { value: 'avoid', label: 'Chci variantu bez kofeinu', description: 'Produkty s kofeinem budou z doporučení vyřazeny', icon: 'coffee-off' }
      ]
    }
  ];
...
`;
  ...
  function availabilityMarkup(live, loadingLabel = 'Ověřujeme cenu a dostupnost…') {
    ...
    return `${live.price ? `<span>${escapeHtml(live.price)}</span>` : '<span>Cena na detailu produktu</span>'}<span class="mbpg__availability${availabilityClass}">${availabilityText}</span>`;
  }

  function productTrustBadges(product) {
    return unique([
      isLabTested(product) ? 'Laboratorně ověřeno' : null,
  ...
  function scoreProduct(product, answers) {
    ...
    if (goalScore >= 80) reasons.push(`velmi dobře odpovídá oblasti „${GOAL_LABELS[answers.goal] || answers.goal}“`);
    else if (goalScore > 0) reasons.push(`odpovídá oblasti „${GOAL_LABELS[answers.goal] || answers.goal}“`);

    if (answers.form && answers.form !== 'any' && productMatchesSelectedForm(product, answers.form)) {
      score += 22;
      if (answers.form === 'capsules') reasons.push('vyhovuje kapslová nebo softgelová forma');
      else if (answers.form === 'mixed') reasons.push('vyhovuje kombinace více forem');
    ...
    if (answers.diet && answers.diet !== 'any' && product[answers.diet] === true) {
      score += 10;
      const dietLabels = {
        vegan: 'splňuje veganské provedení',
        sugarFree: 'je bez cukru',
        glutenFree: 'je bez lepku',
        halal: 'má potvrzené označení halal'
      };
    ...
    if (answers.stimulants === 'avoid' && product.caffeine === false) {
      score += 4;
      reasons.push('neobsahuje kofein');
    }
  ...
  async function enrichRankingWithAvailability(entries, maxResults) {
    ...
    enriched.sort((a, b) => {
      ...
      return b.score - a.score || b.product.priority - a.product.priority || a.product.name.localeCompare(b.product.name, 'cs');
    });
  ...
  async function rankProductsWithSafetyFallback(answers, maxResults) {
    ...
    return {
      ...
      relaxed: fallback.length ? ['forma', 'kofein'] : []
    };
  }
  ...
  class ProductGuide {
        ...
        <section class="mbpg__shell mbpg__intro" aria-labelledby="${this.uid}-title">
          ...
          <div class="mbpg__body">
            ...
            <div class="mbpg__steps" aria-label="Jak průvodce funguje">
              <div class="mbpg__step-row"><span class="mbpg__icon mbpg__step-icon" style="${iconStyleVars('clipboard-check')}" aria-hidden="true">${iconSvg('clipboard-check')}</span><span class="mbpg__step-copy"><strong>Řeknete nám, co hledáte</strong><span>Vyberete hlavní oblast, na kterou se chcete zaměřit.</span></span></div>
              <div class="mbpg__step-row"><span class="mbpg__icon mbpg__step-icon" style="${iconStyleVars('sliders')}" aria-hidden="true">${iconSvg('sliders')}</span><span class="mbpg__step-copy"><strong>Zohledníme vaše preference</strong><span>Forma, provedení a další důležité požadavky zpřesní výběr.</span></span></div>
              <div class="mbpg__step-row"><span class="mbpg__icon mbpg__step-icon" style="${iconStyleVars('package-check')}" aria-hidden="true">${iconSvg('package-check')}</span><span class="mbpg__step-copy"><strong>Ukážeme vhodné produkty a proč</strong><span>Uvidíte hlavní složky, dávkování, cenu i aktuální dostupnost.</span></span></div>
            </div>

            <div class="mbpg__quality-strip" aria-label="Principy MyBears">
              <span>Transparentní složení</span>
              <span>Smysluplné dávky</span>
              <span>U vybraných produktů laboratorní analýzy</span>
            </div>

            <div class="mbpg__actions"><button class="mbpg__button mbpg__button--primary" type="button" data-mbpg-action="start">Najít vhodný produkt</button></div>
    ...
    featuredProductCard(entry) {
      ...
      const specialBadges = productBadges(product)
        .filter((tag) => !['gummies', 'kapsle', 'softgely', 'prášek', 'kombinace forem', 'výhodný balíček', 'laboratorně ověřeno'].includes(tag));
      const labLink = isLabTested(product)
        ? `<a class="mbpg__lab-link" href="${escapeHtml(fullUrl(runtimeConfig.labAnalysesUrl))}">Laboratorní analýzy</a>`
        : '';
      ...
      const addToCart = runtimeConfig.enableAddToCart
        ? (live?.availability === 'out'
          ? `<span class="mbpg__button mbpg__button--ghost" aria-disabled="true">Momentálně nedostupné</span>`
          : `<a class="mbpg__button mbpg__button--primary" href="${escapeHtml(buildCartUrl(product))}" data-mbpg-add="${escapeHtml(product.id)}">Do košíku</a>`)
        : '';
      ...
      return `
        <article class="mbpg__featured-product" data-mbpg-product-id="${escapeHtml(product.id)}" id="${this.uid}-product-${escapeHtml(product.id)}">
          ...
          <div class="mbpg__product-copy">
            <span class="mbpg__match-label">Naše doporučení</span>
            ...
            <div class="mbpg__why">
              <strong>Proč odpovídá vašemu výběru</strong>
            ...
            <table class="mbpg__facts-table">
              ...
              <tbody>
                ...
                <tr><th scope="row">Provedení</th><td>${escapeHtml(specialBadges.join(' · ') || 'bez zvláštního požadavku')}</td></tr>
              </tbody>
            </table>
    ...
    alternativeProductRow(entry, index) {
      ...
      return `
        <article class="mbpg__alternative" data-mbpg-product-id="${escapeHtml(product.id)}" id="${this.uid}-product-${escapeHtml(product.id)}">
          ...
          <span class="mbpg__price-line" data-mbpg-price>${availabilityMarkup(live, 'Ověřujeme…')}</span>
    ...
    comparisonTable(entries) {
      ...
      const rows = [
        ...
        ['Dávkování', (p) => p.dose],
        ['Vystačí', (p) => p.supply],
        ['Laboratorní ověření', (p) => isLabTested(p) ? 'ano – výsledky na webu' : 'bez zveřejněné analýzy v průvodci'],
    ...
    async renderResults() {
      ...
      this.root.innerHTML = `
        <section class="mbpg__shell" aria-labelledby="${this.uid}-loading-title">
          ...
          <div class="mbpg__body mbpg__loading-state">
            ...
            <h2 class="mbpg__title" id="${this.uid}-loading-title">Vyhodnocujeme vaše odpovědi</h2>
            <p class="mbpg__lead">Porovnáváme vhodnost produktů a ověřujeme jejich aktuální cenu a dostupnost.</p>
          </div>
        </section>`;
        ...
        this.root.innerHTML = `
          <section class="mbpg__shell" aria-labelledby="${this.uid}-no-result">
            ...
            <div class="mbpg__body mbpg__empty">
              <h2 class="mbpg__title" id="${this.uid}-no-result">Pro tuto kombinaci nemáme vhodné automatické doporučení</h2>
              <p class="mbpg__lead">Zkuste upravit některou z preferencí nebo si projděte celou nabídku. Průvodce záměrně neukazuje produkt za každou cenu, pokud výběr nedává dostatečný smysl.</p>
      ...
      const fallbackMessage = ranking.fallbackUsed
        ? `<div class="mbpg__notice"><span class="mbpg__notice-icon" aria-hidden="true">${iconSvg('info')}</span><div><strong>Uvolnili jsme vedlejší preference.</strong> Pro zachování smysluplného doporučení jsme nezohlednili: ${escapeHtml(ranking.relaxed.join(', '))}. Hlavní oblast a cílová skupina zůstaly zachované.</div></div>`
        : '';
      const overlapLink = runtimeConfig.overlapCheckerUrl
        ? `<a class="mbpg__button mbpg__button--secondary" href="${escapeHtml(fullUrl(runtimeConfig.overlapCheckerUrl))}">Zkontrolovat překryv účinných látek</a>`
        : '';
        ...
        <section class="mbpg__shell" aria-labelledby="${this.uid}-results-title">
          ...
          <div class="mbpg__body">
            <div class="mbpg__results-head">
              <span class="mbpg__eyebrow">Výsledek průvodce</span>
              <h2 class="mbpg__title" id="${this.uid}-results-title" tabindex="-1">Nejlépe odpovídá vašemu výběru</h2>
              <p class="mbpg__lead">Doporučení vychází z vybrané oblasti, formy a vašich požadavků. Při pořadí zohledňujeme také aktuální dostupnost produktu.</p>
            </div>

            <div class="mbpg__result-summary"><span class="mbpg__notice-icon" aria-hidden="true">${iconSvg('check')}</span><div><strong>Vaše preference</strong>${escapeHtml(answerSummary.join(' · '))}</div></div>

            ${this.featuredProductCard(featured)}

            ${alternatives.length ? `
              <h3 class="mbpg__alternative-title">Další vhodné možnosti</h3>
            ...
            <div class="mbpg__next-tools">
              <div><strong>Už užíváte další doplňky?</strong><span>Ověřte si, zda se vám zbytečně nepřekrývají stejné účinné látky.</span></div>
              ${overlapLink}
            </div>

            <div class="tldr-box"><p><strong>Důležité upozornění:</strong> Průvodce posuzuje produktové parametry a zvolené preference. Nevyhodnocuje diagnózy, léčbu, těhotenství ani individuální potřebu konkrétních látek. Proto například automaticky nedoporučuje doplněk se železem pouze na základě obecné volby „energie“. Před použitím vždy zkontrolujte aktuální etiketu a upozornění na detailu produktu.</p></div>
