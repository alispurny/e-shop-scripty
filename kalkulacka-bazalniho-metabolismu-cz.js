...
(function () {
  ...
  function init() {
    ...
    root.innerHTML = [
      ...
      '    <h2 class="mb-bmr__title" id="mb-bmr-title">BMR kalkulačka pro dospělé</h2>',
      '    <p class="mb-bmr__lead">Spočítejte si orientační klidový energetický výdej podle věku, výšky, hmotnosti a varianty rovnice pro ženy nebo muže.</p>',
      '  </div>',
      '  <div class="mb-bmr__body">',
      '    <p class="mb-bmr__notice"><strong>Důležité:</strong> Výsledek je odhad podle rovnice Mifflin–St Jeor. Nejde o přesné laboratorní měření ani o doporučený minimální příjem kalorií. Kalkulačka je určená pro dospělé od 18 let.</p>',
      '    <form class="mb-bmr__form" novalidate>',
      '      <div class="mb-bmr__grid">',
      '        <div class="mb-bmr__field">',
      '          <label class="mb-bmr__label" for="mb-bmr-sex">Varianta rovnice</label>',
      '          <select class="mb-bmr__select" id="mb-bmr-sex" name="sex">',
      '            <option value="female">Žena</option>',
      '            <option value="male">Muž</option>',
      '          </select>',
      '          <span class="mb-bmr__hint">Rovnice používá dvě varianty podle pohlaví, ve kterém byla původně odvozena.</span>',
      '        </div>',
      '        <div class="mb-bmr__field">',
      '          <label class="mb-bmr__label" for="mb-bmr-age">Věk</label>',
      '          <div class="mb-bmr__input-wrap">',
      '            <input class="mb-bmr__input" id="mb-bmr-age" name="age" type="text" inputmode="decimal" autocomplete="off" placeholder="např. 35" aria-describedby="mb-bmr-age-hint">',
      '            <span class="mb-bmr__unit">let</span>',
      '          </div>',
      '          <span class="mb-bmr__hint" id="mb-bmr-age-hint">Povolené rozmezí: 18–100 let</span>',
      '        </div>',
      '        <div class="mb-bmr__field">',
      '          <label class="mb-bmr__label" for="mb-bmr-height">Výška</label>',
      '          <div class="mb-bmr__input-wrap">',
      '            <input class="mb-bmr__input" id="mb-bmr-height" name="height" type="text" inputmode="decimal" autocomplete="off" placeholder="např. 172" aria-describedby="mb-bmr-height-hint">',
      '            <span class="mb-bmr__unit">cm</span>',
      '          </div>',
      '          <span class="mb-bmr__hint" id="mb-bmr-height-hint">Povolené rozmezí: 100–250 cm</span>',
      '        </div>',
      '        <div class="mb-bmr__field">',
      '          <label class="mb-bmr__label" for="mb-bmr-weight">Hmotnost</label>',
      '          <div class="mb-bmr__input-wrap">',
      '            <input class="mb-bmr__input" id="mb-bmr-weight" name="weight" type="text" inputmode="decimal" autocomplete="off" placeholder="např. 74" aria-describedby="mb-bmr-weight-hint">',
      '            <span class="mb-bmr__unit">kg</span>',
      '          </div>',
      '          <span class="mb-bmr__hint" id="mb-bmr-weight-hint">Povolené rozmezí: 20–400 kg</span>',
      '        </div>',
      '        <div class="mb-bmr__field mb-bmr__field--full">',
      '          <label class="mb-bmr__label" for="mb-bmr-activity">Úroveň aktivity – volitelné</label>',
      '          <select class="mb-bmr__select" id="mb-bmr-activity" name="activity">',
      '            <option value="">Zobrazit pouze klidový výdej</option>',
      '            <option value="1.4">Nízká aktivita (PAL 1,4)</option>',
      '            <option value="1.6">Střední aktivita (PAL 1,6)</option>',
      '            <option value="1.8">Vysoká aktivita (PAL 1,8)</option>',
      '            <option value="2.0">Velmi vysoká aktivita (PAL 2,0)</option>',
      '          </select>',
      '          <span class="mb-bmr__hint">Po výběru se zobrazí také hrubý odhad celkového denního energetického výdeje. Úroveň aktivity bývá obtížné přesně odhadnout.</span>',
      '        </div>',
      '      </div>',
      '      <div class="mb-bmr__error" role="alert" tabindex="-1"></div>',
      '      <div class="mb-bmr__actions">',
      '        <button class="mb-bmr__button mb-bmr__button--primary" type="submit">Vypočítat BMR</button>',
      '        <button class="mb-bmr__button mb-bmr__button--secondary" type="reset">Vymazat údaje</button>',
      '      </div>',
      '    </form>',
      '    <div class="mb-bmr__result" aria-live="polite" hidden>',
      '      <div class="mb-bmr__result-top">',
      '        <div>',
      '          <p class="mb-bmr__result-label">Orientační klidový energetický výdej</p>',
      '          <div class="mb-bmr__score-row"><p class="mb-bmr__score"></p><span class="mb-bmr__score-unit">kcal / den</span></div>',
      '        </div>',
      ...
      '          <p class="mb-bmr__detail-label">Přepočet na kilojouly</p>',
      '          <p class="mb-bmr__detail-value" data-output="kj"></p>',
      '        </div>',
      '        <div class="mb-bmr__detail">',
      '          <p class="mb-bmr__detail-label">Průměr za jednu hodinu</p>',
      ...
      '        <p class="mb-bmr__tdee-label">Orientační celkový denní energetický výdej podle zvoleného PAL</p>',
      ...
      '      <p class="mb-bmr__result-note"><strong>Výsledek není automatický kalorický cíl.</strong> Skutečný výdej se může lišit podle tělesného složení, zdravotního stavu, teploty, spánku, pohybu a dalších faktorů. Pro přesnější měření klidového výdeje se používá nepřímá kalorimetrie.</p>',
      '    </div>',
      '    <p class="mb-bmr__privacy">Výpočet probíhá pouze ve vašem prohlížeči. Zadaný věk, výška, hmotnost ani zvolená varianta rovnice se tímto skriptem nikam neodesílají ani neukládají.</p>',
      '  </div>',
      '</section>'
    ].join('');
    ...
    form.addEventListener('submit', function (event) {
      ...
      if (sex !== 'female' && sex !== 'male') {
        showError('Vyberte variantu rovnice pro ženu nebo muže.');
        return;
      }

      if (!Number.isFinite(age) || age < 18 || age > 100) {
        showError('Zadejte věk v rozmezí 18 až 100 let.');
        ageInput.focus();
        return;
      }

      if (!Number.isFinite(height) || height < 100 || height > 250) {
        showError('Zadejte výšku v rozmezí 100 až 250 cm.');
        heightInput.focus();
        return;
      }

      if (!Number.isFinite(weight) || weight < 20 || weight > 400) {
        showError('Zadejte hmotnost v rozmezí 20 až 400 kg.');
        weightInput.focus();
        return;
      }

      if (pal !== null && [1.4, 1.6, 1.8, 2.0].indexOf(pal) === -1) {
        showError('Vyberte platnou úroveň aktivity.');
        activityInput.focus();
        return;
      }
      ...
      summaryOutput.textContent = 'Podle zadaných údajů vychází orientační klidový energetický výdej přibližně ' + formatNumber(reeRounded, 0) + ' kcal za 24 hodin. Jde o odhad energie spotřebované v klidových podmínkách, nikoliv o přesné laboratorní měření.';
      kjOutput.textContent = formatNumber(kjRounded, 0) + ' kJ / den';
      hourOutput.textContent = formatNumber(hourly, 1) + ' kcal / hod';
      inputsOutput.textContent = formatNumber(age, 0) + ' let · ' + formatNumber(height, 1) + ' cm · ' + formatNumber(weight, 1) + ' kg · ' + sexLabel;
      formulaOutput.textContent = 'Výpočet: 10 × ' + formatNumber(weight, 1) + ' kg + 6,25 × ' + formatNumber(height, 1) + ' cm − 5 × ' + formatNumber(age, 0) + ' let ' + constantText + ' = přibližně ' + formatNumber(reeRounded, 0) + ' kcal / den.';

      if (pal !== null) {
        var tdee = Math.round(ree * pal);
        var selectedText = activityInput.options[activityInput.selectedIndex].text;
        tdeeOutput.textContent = formatNumber(tdee, 0) + ' kcal / den (' + formatNumber(tdee * 4.184, 0) + ' kJ)';
        tdeeNoteOutput.textContent = selectedText + '. Výpočet je BMR/REE × PAL a slouží pouze jako hrubý orientační odhad udržovacího energetického výdeje.';
