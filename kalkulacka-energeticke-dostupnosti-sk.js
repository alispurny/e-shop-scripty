...
`;
  ...
  function init() {
    ...
    root.innerHTML = [
      ...
      '    <h2 class="mb-ea__title" id="mb-ea-title">Kalkulačka energetickej dostupnosti</h2>',
      '    <p class="mb-ea__lead">Odhadnite, koľko energie zostáva organizmu po odpočítaní energetického výdaja pri cvičení vo vzťahu k beztukovej hmote.</p>',
      '  </div>',
      '  <div class="mb-ea__body">',
      '    <p class="mb-ea__notice"><strong>Dôležité:</strong> Kalkulačka nie je diagnostickým nástrojom pre REDs. Výsledok výrazne ovplyvňuje presnosť záznamu jedálnička, odhadu aktívnych kalórií a telesného zloženia.</p>',
      '    <form id="mb-ea-form" novalidate>',
      '      <div class="mb-ea__mode" role="group" aria-label="Spôsob zadania beztukovej hmoty">',
      '        <button type="button" class="mb-ea__mode-btn" id="mb-ea-mode-fat" aria-pressed="true">Hmotnosť a telesný tuk</button>',
      '        <button type="button" class="mb-ea__mode-btn" id="mb-ea-mode-ffm" aria-pressed="false">Poznám beztukovú hmotu</button>',
      '      </div>',
      '      <div class="mb-ea__grid">',
      '        <div class="mb-ea__field"><label class="mb-ea__label" for="mb-ea-sex">Referenčná skupina</label><select class="mb-ea__select" id="mb-ea-sex"><option value="female">Žena</option><option value="male">Muž</option><option value="unspecified">Nechcem uviesť</option></select><span class="mb-ea__hint">Nemení vzorec, iba vysvetlenie výsledku.</span></div>',
      '        <div class="mb-ea__field"><label class="mb-ea__label" for="mb-ea-age">Vek</label><div class="mb-ea__input-wrap"><input class="mb-ea__input" id="mb-ea-age" inputmode="numeric" autocomplete="off" value="30"><span class="mb-ea__unit">rokov</span></div><span class="mb-ea__hint">Určené pre dospelých od 18 rokov.</span></div>',
      '        <div class="mb-ea__conditional" id="mb-ea-fat-fields">',
      '          <div class="mb-ea__grid">',
      '            <div class="mb-ea__field"><label class="mb-ea__label" for="mb-ea-weight">Telesná hmotnosť</label><div class="mb-ea__input-wrap"><input class="mb-ea__input" id="mb-ea-weight" inputmode="decimal" autocomplete="off" value="65"><span class="mb-ea__unit">kg</span></div><span class="mb-ea__hint">Povolené rozmedzie: 30–250 kg</span></div>',
      '            <div class="mb-ea__field"><label class="mb-ea__label" for="mb-ea-fat">Telesný tuk</label><div class="mb-ea__input-wrap"><input class="mb-ea__input" id="mb-ea-fat" inputmode="decimal" autocomplete="off" value="22"><span class="mb-ea__unit">%</span></div><span class="mb-ea__hint">Použite hodnotu z čo najspoľahlivejšieho merania.</span></div>',
      '          </div>',
      '        </div>',
      '        <div class="mb-ea__conditional" id="mb-ea-ffm-fields" hidden>',
      '          <div class="mb-ea__field mb-ea__field--full"><label class="mb-ea__label" for="mb-ea-ffm">Beztuková hmota (FFM)</label><div class="mb-ea__input-wrap"><input class="mb-ea__input" id="mb-ea-ffm" inputmode="decimal" autocomplete="off" value="50,7"><span class="mb-ea__unit">kg</span></div><span class="mb-ea__hint">Napríklad z merania DXA alebo iného vyšetrenia telesného zloženia.</span></div>',
      '        </div>',
      '        <div class="mb-ea__field"><label class="mb-ea__label" for="mb-ea-intake">Priemerný denný energetický príjem</label><div class="mb-ea__input-wrap"><input class="mb-ea__input" id="mb-ea-intake" inputmode="decimal" autocomplete="off" value="2400"><span class="mb-ea__unit">kcal</span></div><span class="mb-ea__hint">Ideálne použite priemer z niekoľkých reprezentatívnych dní.</span></div>',
      '        <div class="mb-ea__field"><label class="mb-ea__label" for="mb-ea-exercise">Priemerný aktívny výdaj pri cvičení</label><div class="mb-ea__input-wrap"><input class="mb-ea__input" id="mb-ea-exercise" inputmode="decimal" autocomplete="off" value="500"><span class="mb-ea__unit">kcal</span></div><span class="mb-ea__hint">Zadajte energiu nad pokojovú úroveň, nie celodenný výdaj.</span></div>',
      '      </div>',
      '      <div class="mb-ea__actions">',
      '        <button type="submit" class="mb-ea__button mb-ea__button--primary">Vypočítať dostupnosť</button>',
      '        <button type="button" class="mb-ea__button mb-ea__button--secondary" id="mb-ea-reset">Vymazať údaje</button>',
      '      </div>',
      ...
      '        <div><p class="mb-ea__result-label">Vaša orientačná energetická dostupnosť</p><p class="mb-ea__score"><span id="mb-ea-score">–</span> <span class="mb-ea__score-unit">kcal/kg FFM/deň</span></p></div>',
      '        <span class="mb-ea__badge" id="mb-ea-badge">–</span>',
      '      </div>',
      '      <p class="mb-ea__summary" id="mb-ea-summary"></p>',
      '      <div class="mb-ea__scale-wrap">',
      '        <p class="mb-ea__scale-title">Orientačné výskumné pásma</p>',
      '        <div class="mb-ea__scale" aria-hidden="true"><span></span><span></span><span></span><i class="mb-ea__marker" id="mb-ea-marker"></i></div>',
      '        <div class="mb-ea__scale-labels"><span>Nízka<br>&lt; 30</span><span>Znížená<br>30–44,9</span><span>Vyššia<br>45 a viac</span></div>',
      '      </div>',
      ...
      '        <div class="mb-ea__metric"><p class="mb-ea__metric-label">Energia po odpočítaní cvičenia</p><p class="mb-ea__metric-value" id="mb-ea-out-remaining">–</p></div>',
      '        <div class="mb-ea__metric"><p class="mb-ea__metric-label">Prepočet na kJ</p><p class="mb-ea__metric-value" id="mb-ea-out-kj">–</p></div>',
      '        <div class="mb-ea__metric"><p class="mb-ea__metric-label">Príjem zodpovedajúci EA 30</p><p class="mb-ea__metric-value" id="mb-ea-out-30">–</p></div>',
      '        <div class="mb-ea__metric"><p class="mb-ea__metric-label">Príjem zodpovedajúci EA 45</p><p class="mb-ea__metric-value" id="mb-ea-out-45">–</p></div>',
      '        <div class="mb-ea__metric"><p class="mb-ea__metric-label">Zadaný aktívny výdaj</p><p class="mb-ea__metric-value" id="mb-ea-out-exercise">–</p></div>',
      '      </div>',
      '      <p class="mb-ea__formula" id="mb-ea-formula"></p>',
      '      <p class="mb-ea__warning" id="mb-ea-warning"></p>',
      '    </section>',
      '    <p class="mb-ea__privacy">Výpočet prebieha iba vo vašom prehliadači. Zadané údaje sa týmto skriptom nikam neodosielajú ani neukladajú.</p>',
      '  </div>',
      '</section>'
    ].join('');
    ...
    function classify(ea) {
      if (ea < 30) return { label: 'Nízka dostupnosť', text: 'Výsledok patrí do pásma tradične spájaného s nízkou energetickou dostupnosťou. Overte správnosť vstupov a pri dlhodobom výskyte alebo príznakoch vyhľadajte športového lekára či nutričného terapeuta.' };
      if (ea < 45) return { label: 'Znížená dostupnosť', text: 'Výsledok sa nachádza v prechodnom pásme. Krátkodobo nemusí znamenať problém, ale opakovaná alebo dlhodobá expozícia môže byť dôležitá pre zdravie, regeneráciu a výkon.' };
      return { label: 'Vyššia dostupnosť', text: 'Výsledok je v pásme, ktoré sa v laboratórnych štúdiách u žien často používalo ako referenčné pre dostatočnú dostupnosť. Samotné číslo však nevylučuje REDs ani iné ťažkosti.' };
    }
    ...
    form.addEventListener('submit', function (event) {
      ...
      if (!Number.isFinite(age) || age < 18 || age > 100) return showError('Zadajte vek v rozmedzí 18–100 rokov.');
      if (!Number.isFinite(intake) || intake < 500 || intake > 10000) return showError('Zadajte priemerný denný príjem v rozmedzí 500–10 000 kcal.');
      if (!Number.isFinite(exercise) || exercise < 0 || exercise > 6000) return showError('Zadajte aktívny výdaj pri cvičení v rozmedzí 0–6 000 kcal za deň.');
      ...
      if (mode === 'fat') {
        ...
        if (!Number.isFinite(weight) || weight < 30 || weight > 250) return showError('Zadajte telesnú hmotnosť v rozmedzí 30–250 kg.');
        if (!Number.isFinite(fat) || fat < 3 || fat > 60) return showError('Zadajte podiel telesného tuku v rozmedzí 3–60 %.');
        ffm = weight * (1 - fat / 100);
      } else {
        ffm = parseNumber(document.getElementById('mb-ea-ffm').value);
        if (!Number.isFinite(ffm) || ffm < 20 || ffm > 180) return showError('Zadajte beztukovú hmotu v rozmedzí 20–180 kg.');
      }

      var remaining = intake - exercise;
      var ea = remaining / ffm;
      if (!Number.isFinite(ea)) return showError('Výpočet sa nepodarilo dokončiť. Skontrolujte zadané údaje.');
      ...
      document.getElementById('mb-ea-out-remaining').textContent = format(round(remaining, 0), 0) + ' kcal/deň';
      document.getElementById('mb-ea-out-kj').textContent = format(round(eaKj, 0), 0) + ' kJ/kg FFM/deň';
      document.getElementById('mb-ea-out-30').textContent = format(round(target30, 0), 0) + ' kcal/deň';
      document.getElementById('mb-ea-out-45').textContent = format(round(target45, 0), 0) + ' kcal/deň';
      document.getElementById('mb-ea-out-exercise').textContent = format(round(exercise, 0), 0) + ' kcal/deň';
      document.getElementById('mb-ea-formula').innerHTML = '<strong>Výpočet:</strong> (' + format(round(intake, 0), 0) + ' − ' + format(round(exercise, 0), 0) + ') ÷ ' + format(round(ffm, 1), 1) + ' = ' + format(round(ea, 1), 1) + ' kcal/kg FFM/deň.';

      var warning = 'Pásma 30 a 45 kcal/kg FFM/deň sú orientačné výskumné referenčné hodnoty, nie diagnostické hranice. Prepočítané príjmy nie sú osobným odporúčaním jedálnička.';
      if (sex === 'male') warning += ' U mužov nie je potvrdená jedna univerzálna hranica nízkej energetickej dostupnosti a odborná interpretácia musí byť mimoriadne opatrná.';
      if (sex === 'unspecified') warning += ' Výskumné pásma boli odvodené najmä zo štúdií žien a nemožno ich rovnako presne vztiahnuť na každého človeka.';
      if (remaining <= 0) warning = 'Zadaný aktívny výdaj je rovnaký alebo vyšší ako energetický príjem. Najprv overte, či ste nezadali celkový výdaj vrátane pokojovej energie namiesto aktívnych kalórií. ' + warning;
