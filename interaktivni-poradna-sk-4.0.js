...
`;
  ...
  function createHub(root) {
    ...
    root.innerHTML = '<div class="mbia__shell"><header class="mbia__hero"><h2 class="mbia__title">Nájdite článok podľa toho, čo práve riešite</h2></header><div class="mbia__body" data-role="body"><div class="mbia__skeleton" aria-label="Načítanie článkov"></div></div></div>';
    ...
    function seriesHtml() {
      ...
      return '<section class="mbia__series" aria-labelledby="' + uid + '-series"><h3 class="mbia__section-title" id="' + uid + '-series">Prejdite si celé tematické série</h3><div class="mbia__series-grid">' + series.map(function (item) {
    ...
    function renderControls() {
      const counts = topicCounts();
      body.innerHTML = '<section class="mbia__search-section" aria-labelledby="' + uid + '-search"><h3 class="mbia__section-title" id="' + uid + '-search">Čo hľadáte?</h3><div class="mbia__search-panel"><div class="mbia__field"><label class="mbia__label" for="' + uid + '-query">Hľadať v článkoch</label><div class="mbia__query-row"><input class="mbia__input" id="' + uid + '-query" type="search" value="' + escapeHtml(state.query) + '" placeholder="Napr. glykémia, horčík, nadúvanie, kĺby…" autocomplete="off" enterkeyhint="search"><button type="button" class="mbia__btn mbia__btn--primary mbia__search-submit" data-search-submit aria-controls="' + uid + '-results" aria-label="Zobraziť články zodpovedajúce zadanému výrazu">Zobraziť články</button></div></div><div class="mbia__field"><label class="mbia__label" for="' + uid + '-intent">Podľa zámeru</label><select class="mbia__select" id="' + uid + '-intent">' + INTENTS.map(function (intent) { return '<option value="' + escapeHtml(intent.id) + '"' + (state.intent === intent.id ? ' selected' : '') + '>' + escapeHtml(intent.label) + '</option>'; }).join('') + '</select></div><div class="mbia__field"><label class="mbia__label" for="' + uid + '-sort">Zoradenie</label><select class="mbia__select" id="' + uid + '-sort"><option value="newest"' + (state.sort === 'newest' ? ' selected' : '') + '>Najnovšie</option><option value="az"' + (state.sort === 'az' ? ' selected' : '') + '>Názov A–Z</option><option value="reading"' + (state.sort === 'reading' ? ' selected' : '') + '>Najkratšie čítanie</option><option value="series"' + (state.sort === 'series' ? ' selected' : '') + (state.topic === 'all' ? ' disabled' : '') + '>Od základov do detailu</option></select></div><div class="mbia__field"><label class="mbia__label">Filtre</label><button type="button" class="mbia__btn mbia__btn--secondary" data-clear style="width:100%">Vymazať výber</button></div></div><div class="mbia__topic-row" role="group" aria-label="Témy článkov"><button type="button" class="mbia__topic" data-topic="all" aria-pressed="' + (state.topic === 'all') + '">Všetky témy <span class="mbia__topic-count">' + state.articles.length + '</span></button>' + TOPICS.map(function (topic) {
      ...
      }).join('') + '</div></section>' + seriesHtml() + '<div id="' + uid + '-results" data-role="results" tabindex="-1" aria-live="polite"></div><div class="mbia__note">Interaktívny prehľad načítava články z hlavného blogu MyBears. Informácie v článkoch majú vzdelávací charakter a nenahrádzajú individuálne vyšetrenie ani odporúčanie lekára.</div>';
      bindControls();
      renderResults();
    }
