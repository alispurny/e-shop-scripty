/**
 * MyBears — sjednocená grafická verze 2.0.2 (CZ)
 ...
 * Version: 2.0.6-cz
 * Verified against MyBears /advisor structure: 2026-08-01
 *
 * Hub mount point:
...
.mbia__panel-head h2, .mbia__panel-head h3, .mbia__ingredient-title { color:#2dc26b !important; font-weight:700 !important; letter-spacing:-.01em; }

/* Complete reset for the main and section headings.
   Upgates applies global H2/H3 typography, shadows and decorations. */
.mbia__shell h2.mbia__title,
#mybears-interactive-advice .mbia__shell h2.mbia__title,
[data-mybears-interactive-advice] .mbia__shell h2.mbia__title {
...
.mbia__shell h3.mbia__section-title,
#mybears-interactive-advice .mbia__shell h3.mbia__section-title,
[data-mybears-interactive-advice] .mbia__shell h3.mbia__section-title {
  display:block !important;
...
/* Search and thematic-series headings are centered.
   The results heading ("Všechny články" / "Všetky články") stays left-aligned. */
.mbia__search-section > .mbia__section-title,
.mbia__series > .mbia__section-title {
  color:#000 !important;
  text-align:center !important;
}
.mbia__toolbar .mbia__section-title {
  color:#000 !important;
  text-align:left !important;
}
.mbia__shell h2.mbia__title::before,
.mbia__shell h2.mbia__title::after,
.mbia__shell h3.mbia__section-title::before,
.mbia__shell h3.mbia__section-title::after,
#mybears-interactive-advice .mbia__shell h2.mbia__title::before,
#mybears-interactive-advice .mbia__shell h2.mbia__title::after,
#mybears-interactive-advice .mbia__shell h3.mbia__section-title::before,
#mybears-interactive-advice .mbia__shell h3.mbia__section-title::after,
[data-mybears-interactive-advice] .mbia__shell h2.mbia__title::before,
[data-mybears-interactive-advice] .mbia__shell h2.mbia__title::after,
[data-mybears-interactive-advice] .mbia__shell h3.mbia__section-title::before,
[data-mybears-interactive-advice] .mbia__shell h3.mbia__section-title::after {
...
/* Article-card title reset.
   The Upgates theme styles H3 elements globally, so every inherited heading
   property must be overridden, not only the text color. */
.mbia__shell h3.mbia__card-title,
#mybears-interactive-advice .mbia__shell h3.mbia__card-title,
[data-mybears-interactive-advice] .mbia__shell h3.mbia__card-title {
  display:block !important;
...
.mbia__shell h3.mbia__card-title::before,
.mbia__shell h3.mbia__card-title::after,
#mybears-interactive-advice .mbia__shell h3.mbia__card-title::before,
#mybears-interactive-advice .mbia__shell h3.mbia__card-title::after,
[data-mybears-interactive-advice] .mbia__shell h3.mbia__card-title::before,
[data-mybears-interactive-advice] .mbia__shell h3.mbia__card-title::after {
  content:none !important;
  display:none !important;
}
.mbia__shell h3.mbia__card-title > a,
#mybears-interactive-advice .mbia__shell h3.mbia__card-title > a,
[data-mybears-interactive-advice] .mbia__shell h3.mbia__card-title > a {
...
.mbia__shell h3.mbia__card-title > a:hover,
#mybears-interactive-advice .mbia__shell h3.mbia__card-title > a:hover,
[data-mybears-interactive-advice] .mbia__shell h3.mbia__card-title > a:hover {
  color:#198d4b !important;
  text-decoration:none !important;
  text-shadow:none !important;
}
/* The same font reset is used for article titles in the related-articles block. */
.mbia-related__item strong {
...
`;
  ...
  const TOPICS = [
    {
      id: 'glykemie', label: 'Glykémie a metabolismus', icon: '◒',
      keywords: ['glykem', 'cukr v krvi', 'hba1c', 'prediabet', 'diabet', 'inzulin', 'insulin', 'homa-ir', 'glykemický index', 'glykemická nálož', 'metabolický syndrom', 'triglycerid', 'obvod pasu']
    },
    {
      id: 'cholesterol', label: 'Cholesterol a srdce', icon: '♡',
      keywords: ['cholesterol', 'ldl', 'hdl', 'triglycerid', 'kardiovask', 'srdce', 'krevní tlak', 'omega 3', 'omega-3']
    },
    {
      id: 'traveni', label: 'Trávení a mikroflóra', icon: '◌',
      keywords: ['tráven', 'nadým', 'nafoukl', 'zácp', 'průjem', 'střev', 'mikroflór', 'probiotik', 'prebiotik', 'vláknin', 'jablečný ocet', 'břich']
    },
    {
      id: 'spanek-stres', label: 'Spánek, stres a psychika', icon: '☾',
      keywords: ['spánek', 'spaní', 'nespav', 'kortizol', 'stres', 'relax', 'psychik', 'úzkost', 'nálad', 'meduňk', '5-htp', 'serotonin', 'noční buzení']
    },
    {
      id: 'klouby-kosti', label: 'Klouby, kosti a pohyb', icon: '◇',
    ...
    },
    {
      ...
      keywords: ['pokož', 'pleť', 'kůž', 'kožní', 'akné', 'kolagen', 'vlasy', 'nehty', 'beauty', 'hyaluron', 'hydratace pleti', 'spfn', 'spf']
    },
    {
      id: 'vitaminy-mineraly', label: 'Vitamíny a minerály', icon: '＋',
      keywords: ['vitamín', 'vitamin', 'minerál', 'hořčík', 'zinek', 'železo', 'selen', 'biotin', 'b12', 'b6', 'd3', 'k2', 'folát', 'kyselina listová', 'cholin', 'koenzym q10']
    },
    {
      id: 'sport-vykon', label: 'Sport, energie a výkon', icon: '△',
      keywords: ['sport', 'běh', 'běhání', 'trénink', 'výkon', 'regenerac', 'sval', 'pre-workout', 'preworkout', 'kofein', 'energie', 'e-sport', 'gamer', 'soustředění']
    },
    {
    ...
    {
      id: 'deti-rodina', label: 'Děti a rodina', icon: '●',
      keywords: ['děti', 'dítě', 'děts', 'rodina', 'škol', 'pediatr']
    },
    {
      id: 'zivotni-styl', label: 'Životní styl a prevence', icon: '○',
      keywords: ['pitný režim', 'dehydrat', 'longevity', 'dlouhověk', 'prevence', 'hubnutí', 'hmotnost', 'jídelníček', 'strava', 'kancelář', 'únava', 'režim', 'zdravý životní styl']
    },
    {
      id: 'slozeni-doplnku', label: 'Složení a výběr doplňků', icon: '≡',
      keywords: ['jak vybrat', 'rozdíl mezi', 'složení', 'etiket', 'doplň', 'gummies', 'bez cukru', 'sladid', 'maltitol', 'erytritol', 'protispékav', 'stearan', 'e551', 'vstřebatel', 'forma látky']
    }
  ];

  const INTENTS = [
    { id: 'all', label: 'Všechny články', keywords: [] },
    { id: 'understand', label: 'Chci porozumět hodnotám a pojmům', keywords: ['hodnot', 'co je', 'co znamená', 'rozdíl', 'příznaky', 'diagnost', 'vyšetřen', 'test', 'hba1c', 'ldl', 'hdl', 'homa-ir'] },
    { id: 'problem', label: 'Řeším konkrétní potíž', keywords: ['bolest', 'únava', 'nadým', 'zácpa', 'nespav', 'buzení', 'suchá', 'dehydrat', 'akné', 'zarudnutí', 'chuť na sladké', 'mozková mlha'] },
    { id: 'choose', label: 'Chci lépe vybrat doplněk', keywords: ['jak vybrat', 'složení', 'dávkování', 'forma', 'porovn', 'vs.', 'rozdíl mezi', 'kdy brát', 'jak užívat', 'doplň'] },
    { id: 'routine', label: 'Chci zlepšit každodenní rutinu', keywords: ['tipy', 'rutina', 'prevence', 'jídelníček', 'pohyb', 'pitný režim', 'spánek', 'regenerace', 'jak často', 'co pomáhá'] }
  ];
  ...
  const PILLAR_PATTERNS = {
    ...
    'klouby-kosti': ['kloubni-vyziva', 'bolest-kloubu', 'osteoartroza'],
  ...
  const FALLBACK_ARTICLES = [
    {
      title: 'Glykémie a cukr v krvi: normální hodnoty nalačno, po jídle a HbA1c',
      url: '/advisor/glykemie-cukr-v-krvi-hodnoty-nalacno-po-jidle-hba1c',
      description: 'Přehled hodnot glykémie nalačno, po jídle, OGTT a HbA1c včetně vysvětlení domácího a laboratorního měření.'
    },
    {
      title: 'Prediabetes: hodnoty cukru v krvi, příznaky a co lze změnit',
      url: '/advisor/prediabetes-hodnoty-priznaky-co-delat',
      description: 'Co znamená prediabetes, jak se vyšetřuje a které změny životního stylu mají smysl.'
    },
    {
      title: 'Inzulinová rezistence: příznaky, vyšetření, HOMA-IR a léčba',
      url: '/advisor/inzulinova-rezistence-priznaky-homa-ir-lecba',
      description: 'Vysvětlení inzulinové rezistence, HOMA-IR a souvislostí s pohybem, stravou, spánkem a hmotností.'
    },
    {
      title: 'Metabolický syndrom: jak spolu souvisejí obvod pasu, tlak, cukr a triglyceridy',
      url: '/advisor/metabolicky-syndrom-priznaky-kriteria-lecba',
      description: 'Pět kritérií metabolického syndromu, jejich vzájemné souvislosti a praktický plán změn.'
    },
    {
      title: 'Kloubní výživa: které složky fungují a proč je kombinovat',
      url: '/advisor/kloubni-vyziva-ktere-slozky-funguji-a-proc-je-kombinovat',
      description: 'Přehled kolagenu, glukosaminu, chondroitinu, MSM, kyseliny hyaluronové a rostlinných extraktů.'
    },
    {
      title: 'Bolest kloubů: nejčastější příčiny a kdy zpozornět',
      url: '/advisor/bolest-kloubu-priciny-kdy-k-lekari',
      description: 'Nejčastější příčiny bolesti kloubů, varovné signály a praktická péče při mírných obtížích.'
    },
    {
      title: 'Kolagen na pokožku: kdy má smysl a co od něj čekat',
      url: '/advisor/kolagen-na-pokozku-kdy-ma-smysl',
      description: 'Realistický pohled na kolagen, SPF, hydrataci a dlouhodobou beauty rutinu.'
    },
    {
      title: 'Pitný režim: kolik pít a jak poznat dehydrataci',
      url: '/advisor/pitny-rezim-kolik-pit-jak-poznat-dehydrataci',
      description: 'Kolik tekutin potřebujete, co se počítá do pitného režimu a jak poznat první známky dehydratace.'
    },
    {
      title: 'Vitamín B12: příznaky nedostatku, testy a jak doplňovat',
      url: '/advisor/vitamin-b12-priznaky-nedostatku-testy-a-jak-doplnovat',
      description: 'Rizikové skupiny, laboratorní vyšetření, zdroje B12 a praktické možnosti doplňování.'
    },
    {
      title: 'Kortizol a spánek: proč se v noci budíte a ráno jste unavení',
      url: '/advisor/kortizol-a-spanek-proc-se-budite-v-noci',
      description: 'Souvislosti stresu, večerního přebuzení, nočního buzení a ranní únavy.'
    },
    {
      title: 'Gummies bez cukru: maltitol, erytritol a jak to funguje',
      url: '/advisor/vitaminy-bez-cukru-co-znamena-bez-cukru-u-gummies',
      description: 'Jak číst označení bez cukru, jak se liší sladidla a které informace na etiketě sledovat.'
    },
    {
      title: 'Longevity: 5 pilířů, jak žít déle a zdravěji',
      url: '/advisor/longevity-jak-snizit-riziko-civilizacnich-nemoci-a-zit-dele',
      description: 'Pohyb, spánek, strava, prevence a realistická role doplňků v dlouhodobé rutině.'
    }
  ];
...
.mbia__query-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:stretch}
.mbia__search-submit{min-width:150px;white-space:nowrap;padding-inline:16px}
...
.mbia__featured{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-bottom:18px}
.mbia__featured .mbia__card{display:flex;flex-direction:column;grid-column:auto;grid-row:auto;height:100%}
.mbia__featured .mbia__card:first-child{grid-column:auto;grid-row:auto}
.mbia__featured .mbia__card .mbia__image-wrap,
.mbia__featured .mbia__card:first-child .mbia__image-wrap,
.mbia__featured .mbia__card:not(:first-child) .mbia__image-wrap{width:100%;height:auto;min-height:0;aspect-ratio:16/9;border-radius:0}
.mbia__featured .mbia__card .mbia__card-body,
.mbia__featured .mbia__card:not(:first-child) .mbia__card-body{padding:16px}
.mbia__featured .mbia__card .mbia__description,
.mbia__featured .mbia__card:not(:first-child) .mbia__description{display:-webkit-box}
.mbia__grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}
.mbia__card{position:relative;display:flex;flex-direction:column;min-width:0;height:100%;border:1px solid var(--mb-border);border-radius:11px;background:#fff;overflow:hidden;transition:transform .15s ease,border-color .15s ease,box-shadow .15s ease}
...
@media (max-width:980px){.mbia__search-panel{grid-template-columns:1fr 1fr}.mbia__series-grid{grid-template-columns:repeat(2,1fr)}.mbia__featured,.mbia__grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media (max-width:650px){.mbia{margin:14px 0}.mbia__topic-row{flex-wrap:nowrap;overflow-x:auto;scrollbar-width:thin}.mbia__shell{border-radius:10px}.mbia__hero{padding:22px 18px}.mbia__title{font-size:25px}.mbia__lead{font-size:15px}.mbia__body{padding:18px}.mbia__search-panel{grid-template-columns:1fr;padding:12px}.mbia__query-row{grid-template-columns:1fr}.mbia__search-submit{width:100%;min-width:0}.mbia__series-grid,.mbia__featured,.mbia__grid,.mbia-related__grid{grid-template-columns:1fr}.mbia__toolbar{align-items:flex-start}.mbia__card-actions{flex-direction:column}.mbia__btn{width:100%}.mbia-toc{padding:15px}}
...
`;
  ...
  function findDescription(container, title) {
    ...
    const raw = normalizeText(container.textContent)
      ...
      .replace(/Pokračovat na celý článek/gi, '')
      .trim();
  ...
  function parseAdvisorIndex(html) {
    ...
    links.forEach(function (link) {
      ...
      if (title.length < 8 || /pokračovat|celý článek/i.test(title)) return;
  ...
  function fetchAdvisorIndex() {
    ...
    const promise = fetch(url, { credentials: 'same-origin', headers: { Accept: 'text/html' } })
      ...
      .then(function (html) {
        ...
        if (articles.length < 3) throw new Error('Na stránce blogu nebyl nalezen dostatečný počet článků.');
        return articles;
      })
  ...
  function createHub(root) {
    ...
    root.innerHTML = '<div class="mbia__shell"><header class="mbia__hero"><h2 class="mbia__title">Najděte článek podle toho, co právě řešíte</h2></header><div class="mbia__body" data-role="body"><div class="mbia__skeleton" aria-label="Načítání článků"></div></div></div>';
    ...
    function filteredArticles() {
      ...
      result = result.slice();
      if (state.sort === 'az') result.sort(function (a, b) { return a.title.localeCompare(b.title, 'cs-CZ'); });
    ...
    function cardHtml(article, featured) {
      ...
      const reading = view.readingMinutes ? view.readingMinutes + ' min čtení' : '';
      const isNew = article.order < 4 && !state.query && state.topic === 'all' && state.intent === 'all';
      return '<article class="mbia__card" data-article-url="' + escapeHtml(article.url) + '"><div class="mbia__image-wrap">' + image + (isNew ? '<span class="mbia__new">Nově v poradně</span>' : '') + '</div><div class="mbia__card-body"><div class="mbia__card-topics">' + topics + '</div><h3 class="mbia__card-title"><a href="' + escapeHtml(article.url) + '">' + escapeHtml(article.title) + '</a></h3>' + (article.description ? '<p class="mbia__description">' + escapeHtml(article.description) + '</p>' : '') + '<div class="mbia__card-meta">' + (date ? '<span>Publikováno ' + escapeHtml(date) + '</span>' : '') + (reading ? '<span>' + escapeHtml(reading) + '</span>' : '') + '</div><div class="mbia__card-actions"><a class="mbia__btn mbia__btn--primary" href="' + escapeHtml(article.url) + '">Přečíst článek</a></div></div></article>';
    }
    ...
    function seriesHtml() {
      ...
      return '<section class="mbia__series" aria-labelledby="' + uid + '-series"><h3 class="mbia__section-title" id="' + uid + '-series">Projděte si celé tematické série</h3><div class="mbia__series-grid">' + series.map(function (item) {
        return '<button type="button" class="mbia__series-btn" data-series="' + escapeHtml(item.topic.id) + '"><span class="mbia__series-name">' + escapeHtml(item.topic.icon + ' ' + item.topic.label) + '</span><span class="mbia__series-info">' + item.count + ' článků v tématu</span></button>';
      }).join('') + '</div></section>';
    }

    function renderControls() {
      const counts = topicCounts();
      body.innerHTML = '<section class="mbia__search-section" aria-labelledby="' + uid + '-search"><h3 class="mbia__section-title" id="' + uid + '-search">Co hledáte?</h3><div class="mbia__search-panel"><div class="mbia__field"><label class="mbia__label" for="' + uid + '-query">Hledat v článcích</label><div class="mbia__query-row"><input class="mbia__input" id="' + uid + '-query" type="search" value="' + escapeHtml(state.query) + '" placeholder="Např. glykémie, hořčík, nadýmání, klouby…" autocomplete="off" enterkeyhint="search"><button type="button" class="mbia__btn mbia__btn--primary mbia__search-submit" data-search-submit aria-controls="' + uid + '-results" aria-label="Zobrazit články odpovídající zadanému výrazu">Zobrazit články</button></div></div><div class="mbia__field"><label class="mbia__label" for="' + uid + '-intent">Podle záměru</label><select class="mbia__select" id="' + uid + '-intent">' + INTENTS.map(function (intent) { return '<option value="' + escapeHtml(intent.id) + '"' + (state.intent === intent.id ? ' selected' : '') + '>' + escapeHtml(intent.label) + '</option>'; }).join('') + '</select></div><div class="mbia__field"><label class="mbia__label" for="' + uid + '-sort">Řazení</label><select class="mbia__select" id="' + uid + '-sort"><option value="newest"' + (state.sort === 'newest' ? ' selected' : '') + '>Nejnovější</option><option value="az"' + (state.sort === 'az' ? ' selected' : '') + '>Název A–Z</option><option value="reading"' + (state.sort === 'reading' ? ' selected' : '') + '>Nejkratší čtení</option><option value="series"' + (state.sort === 'series' ? ' selected' : '') + (state.topic === 'all' ? ' disabled' : '') + '>Od základů do detailu</option></select></div><div class="mbia__field"><label class="mbia__label">Filtry</label><button type="button" class="mbia__btn mbia__btn--secondary" data-clear style="width:100%">Vymazat výběr</button></div></div><div class="mbia__topic-row" role="group" aria-label="Témata článků"><button type="button" class="mbia__topic" data-topic="all" aria-pressed="' + (state.topic === 'all') + '">Všechna témata <span class="mbia__topic-count">' + state.articles.length + '</span></button>' + TOPICS.map(function (topic) {
      ...
      }).join('') + '</div></section>' + seriesHtml() + '<div id="' + uid + '-results" data-role="results" tabindex="-1" aria-live="polite"></div><div class="mbia__note">Interaktivní přehled načítá články z hlavního blogu MyBears. Informace v článcích mají vzdělávací charakter a nenahrazují individuální vyšetření ani doporučení lékaře.</div>';
      bindControls();
      renderResults();
    }
    ...
    function bindControls() {
      ...
      const searchSubmit = body.querySelector('[data-search-submit]');
      let timer = null;
      ...
      function scrollToResultsOnMobile() {
        ...
        window.requestAnimationFrame(function () {
          ...
          try { results.focus({ preventScroll: true }); } catch (error) { results.focus(); }
        });
      }

      function applyQuery(shouldScroll) {
    ...
    function renderResults() {
      ...
      const heading = activeTopic ? activeTopic.label : state.query ? 'Výsledky hledání' : 'Všechny články';
      let html = '<div class="mbia__toolbar"><div><h3 class="mbia__section-title" style="margin-bottom:3px">' + escapeHtml(heading) + '</h3><div class="mbia__result-count">Nalezeno <strong>' + result.length + '</strong> článků</div></div>' + ((state.query || state.topic !== 'all' || state.intent !== 'all') ? '<button type="button" class="mbia__clear" data-results-clear>Zrušit filtry</button>' : '') + '</div>';
      if (!result.length) {
        html += '<div class="mbia__empty"><h4 class="mbia__empty-title">Pro tuto kombinaci jsme článek nenašli</h4><p>Zkuste obecnější výraz nebo zrušte některý filtr. Můžete také přejít na kompletní <a href="' + escapeHtml(CONFIG.advisorIndexUrl) + '">blog MyBears</a>.</p></div>';
      } else {
        if (regular.length) html += '<div class="mbia__grid">' + regular.map(function (article) { return cardHtml(article, false); }).join('') + '</div>';
        if (state.visible < result.length) html += '<div class="mbia__load-more-wrap"><button type="button" class="mbia__btn mbia__btn--secondary" data-load-more>Zobrazit další články (' + Math.min(Number(CONFIG.articlesPerPage) || 12, result.length - state.visible) + ')</button></div>';
      }
  ...
  function installRelatedArticles(content) {
    ...
    fetchAdvisorIndex().then(function (articles) {
      ...
      section.innerHTML = '<h2 class="mbia-related__title">Pokračujte v tématu</h2><p class="mbia-related__lead">Vybrali jsme další články, které přirozeně navazují na právě přečtené téma.</p><div class="mbia-related__grid">' + related.map(function (article) {
        const topic = topicById(article.topics[0]);
        return '<a class="mbia-related__item" href="' + escapeHtml(article.url) + '"><strong>' + escapeHtml(article.title) + '</strong><span>' + escapeHtml(topic ? topic.label : 'Poradna MyBears') + '</span></a>';
      }).join('') + '</div>';
  ...
  function init() {
    ...
    document.querySelectorAll(ROOT_SELECTOR).forEach(function (root) {
      const mountedVersion = root.getAttribute('data-mbia-version');
      if (root.getAttribute('data-mbia-mounted') === 'true' && mountedVersion === VERSION) return;
      if (root.getAttribute('data-mbia-mounted') === 'true' && mountedVersion !== VERSION) {
        root.innerHTML = '';
        root.removeAttribute('data-mbia-mounted');
      }
