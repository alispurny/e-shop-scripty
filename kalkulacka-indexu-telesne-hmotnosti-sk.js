...
(function () {
  ...
  /*
   * V4 zámerne prepíše aj staršiu inštanciu kalkulačky. Vďaka tomu nebude
   * pôvodný skript s chybnou stupnicou uprednostnený, ak ho stránka alebo cache
   * načíta skôr než tento súbor.
   */
  ...
  function getCategory(bmi) {
    if (bmi < 18.5) {
      return {
        label: 'Podváha',
        key: 'underweight',
        summary: 'Výsledok patrí do pásma podváhy. BMI samo neukazuje príčinu nižšej hmotnosti ani stav výživy.'
      };
    }

    if (bmi < 25) {
      return {
        label: 'Bežné rozmedzie',
        key: 'healthy',
        summary: 'Výsledok patrí do bežného referenčného rozmedzia BMI pre dospelých. Celkový zdravotný stav však nemožno posúdiť iba podľa BMI.'
      };
    }
    ...
    if (bmi < 30) {
      return {
        label: 'Nadváha',
        key: 'overweight',
        summary: 'Výsledok patrí do pásma nadváhy. Na presnejšie posúdenie má zmysel zohľadniť aj obvod pása, telesné zloženie a ďalšie zdravotné ukazovatele.'
      };
    }

    if (bmi < 35) {
      return {
        label: 'Obezita Aj. stupňa',
        key: 'obesity-1',
        summary: 'Výsledok patrí do pásma obezity Aj. stupňa. BMI je orientačný skríningový údaj, nie samostatná diagnóza.'
      };
    }

    if (bmi < 40) {
      return {
        label: 'Obezita II. stupňa',
        key: 'obesity-2',
        summary: 'Výsledok patrí do pásma obezity II. stupňa. Individuálny význam je vhodné posúdiť spolu s lekárom a ďalšími zdravotnými ukazovateľmi.'
      };
    }

    return {
      label: 'Obezita III. stupňa',
      key: 'obesity-3',
      summary: 'Výsledok patrí do pásma obezity III. stupňa. Na bezpečné posúdenie zdravotného stavu a ďalšieho postupu je vhodná konzultácia s lekárom.'
    };
  }
  ...
  function markerPosition(bmi) {
    ...
    /* Presné hranice farebných úsekov na stupnici 14 až 40. */
  ...
  root.innerHTML = [
    ...
    '    <h2 class="mb-bmi__title" id="mb-bmi-title">BMI kalkulačka pre dospelých</h2>',
    '    <p class="mb-bmi__lead">Zadajte svoju výšku a hmotnosť. Kalkulačka vypočíta BMI, orientačnú kategóriu a hmotnostné rozmedzie zodpovedajúce BMI 18,5–24,9.</p>',
    '  </div>',
    '  <div class="mb-bmi__body">',
    '    <p class="mb-bmi__notice"><strong>Dôležité:</strong> Kalkulačka je určená pre dospelých od 18 rokov. Nie je určená pre deti, dospievajúcich ani na hodnotenie hmotnosti v tehotenstve.</p>',
    ...
    '            <input class="mb-bmi__input" id="mb-bmi-height" name="height" type="text" inputmode="decimal" autocomplete="off" placeholder="napr. 172" aria-describedby="mb-bmi-height-hint">',
    '            <span class="mb-bmi__unit" aria-hidden="true">cm</span>',
    '          </div>',
    '          <span class="mb-bmi__hint" id="mb-bmi-height-hint">Povolené rozmedzie: 100–250 cm</span>',
    '        </div>',
    '        <div class="mb-bmi__field">',
    '          <label class="mb-bmi__label" for="mb-bmi-weight">Hmotnosť</label>',
    '          <div class="mb-bmi__input-wrap">',
    '            <input class="mb-bmi__input" id="mb-bmi-weight" name="weight" type="text" inputmode="decimal" autocomplete="off" placeholder="napr. 74,5" aria-describedby="mb-bmi-weight-hint">',
    '            <span class="mb-bmi__unit" aria-hidden="true">kg</span>',
    '          </div>',
    '          <span class="mb-bmi__hint" id="mb-bmi-weight-hint">Povolené rozmedzie: 20–400 kg</span>',
    '        </div>',
    '      </div>',
    '      <div class="mb-bmi__error" id="mb-bmi-error" role="alert" aria-live="assertive"></div>',
    '      <div class="mb-bmi__actions">',
    '        <button class="mb-bmi__button mb-bmi__button--primary" type="submit">Vypočítať BMI</button>',
    '        <button class="mb-bmi__button mb-bmi__button--secondary" type="reset">Vymazať údaje</button>',
    '      </div>',
    '    </form>',
    '    <section class="mb-bmi__result" id="mb-bmi-result" aria-live="polite" tabindex="-1" hidden>',
    '      <div class="mb-bmi__result-top">',
    '        <div>',
    '          <p class="mb-bmi__result-label">Vaše orientačné BMI</p>',
    ...
    '        <p class="mb-bmi__scale-title">Orientačné zaradenie výsledku</p>',
    ...
    '          <span>Bežné<br>18,5–24,9</span>',
    '          <span>Nadváha<br>25–29,9</span>',
    '          <span>Obezita<br>30 a viac</span>',
    '        </div>',
    '      </div>',
    '      <div class="mb-bmi__details">',
    '        <div class="mb-bmi__detail">',
    '          <p class="mb-bmi__detail-label">Orientačné hmotnostné rozmedzie</p>',
    ...
    '      <p class="mb-bmi__result-note"><strong>Výsledok je orientačný.</strong> BMI nerozlišuje tukovú, svalovú a kostnú hmotu a samo osebe neurčuje zdravotný stav. Pri nechcenom úbytku či náraste hmotnosti alebo zdravotných ťažkostiach sa obráťte na lekára.</p>',
    '    </section>',
    '    <p class="mb-bmi__privacy">Výpočet prebieha iba vo vašom prehliadači. Zadaná výška ani hmotnosť sa týmto skriptom nikam neodosielajú ani neukladajú.</p>',
    '  </div>',
    '</section>'
  ].join('\n');
  ...
  form.addEventListener('submit', function (event) {
    ...
    if (!Number.isFinite(heightCm) || !Number.isFinite(weightKg)) {
      showError('Vyplňte, prosím, výšku aj hmotnosť ako číslo. Môžete použiť desatinnú čiarku alebo bodku.');
      return;
    }

    if (heightCm < 100 || heightCm > 250) {
      showError('Skontrolujte zadanú výšku. Kalkulačka prijíma hodnoty od 100 do 250 cm.');
      heightInput.focus();
      return;
    }

    if (weightKg < 20 || weightKg > 400) {
      showError('Skontrolujte zadanú hmotnosť. Kalkulačka prijíma hodnoty od 20 do 400 kg.');
      weightInput.focus();
      return;
    }
