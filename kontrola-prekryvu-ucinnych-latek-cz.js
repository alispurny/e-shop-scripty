...
`;
  ...
  function createApp(root) {
    ...
    async function hydrateProduct(product) {
      ...
      try {
        ...
        if (selectedNode && data.price) {
          ...
          if (priceNode) priceNode.textContent = 'Aktuální cena: ' + data.price;
        }
      } catch (error) {
    ...
    function selectedCard(p) {
      ...
      return '<div class="mboc__selected" data-selected-id="' + escapeHtml(p.id) + '"><div class="mboc__selected-top"><div><h3 class="mboc__selected-name">' + escapeHtml(p.name) + '</h3><div class="mboc__product-meta" data-role="live-price">' + (hydration && hydration.price ? 'Aktuální cena: ' + escapeHtml(hydration.price) : escapeHtml(p.package)) + '</div></div><button type="button" class="mboc__remove" data-action="remove" data-product-id="' + escapeHtml(p.id) + '" aria-label="Odebrat ' + escapeHtml(p.name) + '">×</button></div><label class="mboc__dose-label" for="' + uid + '-dose-' + escapeHtml(p.id) + '">Denní dávka</label><select class="mboc__dose" id="' + uid + '-dose-' + escapeHtml(p.id) + '" data-action="dose" data-product-id="' + escapeHtml(p.id) + '">' + p.doses.map(function (d) { return '<option value="' + escapeHtml(d.id) + '"' + (d.id === doseId ? ' selected' : '') + '>' + escapeHtml(d.label) + '</option>'; }).join('') + '</select></div>';
    }
