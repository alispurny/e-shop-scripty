...
`;
  ...
  function productIcon(product) {
    if (product.category === 'Gumené medvedíky') return '●';
    if (product.category === 'Kolagény') return '✦';
    return '◆';
  }

  function mockupPalette(product) {
    if (product.category === 'Kolagény') return { top: '#DBC442', body: '#fff8dd', accent: '#f0d66b', text: '#000000' };
    if (product.category === 'Kapsuly a softgely') return { top: '#DBC442', body: '#f5fbf6', accent: '#cfeeda', text: '#000000' };
  ...
  function makeProductMockup(product) {
    ...
    const badge = product.category === 'Kolagény' ? 'Kolagén' : (product.category === 'Kapsuly a softgely' ? 'Kapsuly' : 'Gumené medvedíky');
    const svg = [
  ...
  function createApp(root) {
    ...
    function resultMarkup() {
      ...
      const warnings = calc.warnings.length ? '<div class="mboc__warnings"><h2 class="mboc__section-title">Dôležité upozornenia</h2>' + calc.warnings.map(function (w) { return '<div class="mboc__warning' + (w.danger ? ' is-danger' : '') + '"><strong>' + escapeHtml(w.title) + '</strong>' + escapeHtml(w.text) + '</div>'; }).join('') + '</div>' : '';
