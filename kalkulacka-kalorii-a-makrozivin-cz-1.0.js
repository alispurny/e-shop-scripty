/**
 * MyBears — kalkulačka kalorií a makroživin (CZ)
 * Sjednocený vizuální systém podle finální šablony převodníku krevních lipidů.
 * Funkční logika, výpočtové vztahy, interní ID a způsob vložení zůstávají zachované.
 *
 * Mount point:
...
`;
    ...
    root.innerHTML = [
      ...
      '    <h2 class="mb-cm__title" id="mb-cm-title">Kalkulačka kalorií a makroživin</h2>',
      '    <p class="mb-cm__lead">Odhadněte denní energetický příjem podle cíle a rozdělte jej na bílkoviny, tuky a sacharidy.</p>',
      '  </div>',
      '  <div class="mb-cm__body">',
      '    <p class="mb-cm__notice"><strong>Důležité:</strong> Výsledek je modelový odhad pro zdravé dospělé od 18 let. Nejde o léčebný jídelníček ani individuální nutriční doporučení.</p>',
      '    <form id="mb-cm-form" novalidate>',
      '      <div class="mb-cm__grid">',
      '        <div class="mb-cm__field">',
      '          <label class="mb-cm__label" for="mb-cm-sex">Varianta rovnice</label>',
      '          <select class="mb-cm__select" id="mb-cm-sex">',
      '            <option value="female">Žena</option>',
      '            <option value="male">Muž</option>',
      '          </select>',
      '          <span class="mb-cm__hint">Rovnice Mifflin–St Jeor používá dvě rozdílné konstanty.</span>',
      '        </div>',
      '        <div class="mb-cm__field">',
      '          <label class="mb-cm__label" for="mb-cm-age">Věk</label>',
      '          <div class="mb-cm__input-wrap"><input class="mb-cm__input" id="mb-cm-age" inputmode="numeric" autocomplete="off" value="35" aria-describedby="mb-cm-age-hint"><span class="mb-cm__unit">let</span></div>',
      '          <span class="mb-cm__hint" id="mb-cm-age-hint">Povolené rozmezí: 18–100 let</span>',
      '        </div>',
      '        <div class="mb-cm__field">',
      ...
      '          <span class="mb-cm__hint" id="mb-cm-height-hint">Povolené rozmezí: 100–250 cm</span>',
      '        </div>',
      '        <div class="mb-cm__field">',
      ...
      '          <span class="mb-cm__hint" id="mb-cm-weight-hint">Povolené rozmezí: 30–400 kg</span>',
      '        </div>',
      '        <div class="mb-cm__field">',
      '          <label class="mb-cm__label" for="mb-cm-activity">Úroveň aktivity (PAL)</label>',
      '          <select class="mb-cm__select" id="mb-cm-activity">',
      '            <option value="1.4">Nízká aktivita (PAL 1,4)</option>',
      '            <option value="1.6" selected>Střední aktivita (PAL 1,6)</option>',
      '            <option value="1.8">Vysoká aktivita (PAL 1,8)</option>',
      '            <option value="2.0">Velmi vysoká aktivita (PAL 2,0)</option>',
      '          </select>',
      '          <span class="mb-cm__hint">PAL je zjednodušený odhad celodenní pohybové aktivity.</span>',
      '        </div>',
      '        <div class="mb-cm__field">',
      '          <label class="mb-cm__label" for="mb-cm-goal">Cíl</label>',
      '          <select class="mb-cm__select" id="mb-cm-goal">',
      '            <option value="loss">Snížení hmotnosti</option>',
      '            <option value="maintain" selected>Udržení hmotnosti</option>',
      '            <option value="gain">Zvýšení hmotnosti</option>',
      '          </select>',
      '          <span class="mb-cm__hint">Cíl upravuje vypočtené TDEE o zvolené procento.</span>',
      '        </div>',
      ...
      '          <span class="mb-cm__hint">Procento je výpočtový scénář, nikoliv záruka konkrétní změny hmotnosti.</span>',
      '        </div>',
      '      </div>',
      '      <details class="mb-cm__advanced">',
      '        <summary>Pokročilé nastavení makroživin</summary>',
      '        <div class="mb-cm__advanced-body">',
      '          <div class="mb-cm__grid">',
      '            <div class="mb-cm__field">',
      '              <label class="mb-cm__label" for="mb-cm-protein">Bílkoviny na kg hmotnosti</label>',
      '              <select class="mb-cm__select" id="mb-cm-protein">',
      '                <option value="0.83">0,83 g/kg – referenční příjem EFSA</option>',
      '                <option value="1.2">1,2 g/kg – zvýšený příjem</option>',
      '                <option value="1.6" selected>1,6 g/kg – aktivní nebo redukční režim</option>',
      '                <option value="2.0">2,0 g/kg – vysoký příjem</option>',
      '              </select>',
      '              <span class="mb-cm__hint">Výpočet používá aktuální hmotnost, nikoliv cílovou hmotnost.</span>',
      '            </div>',
      '            <div class="mb-cm__field">',
      '              <label class="mb-cm__label" for="mb-cm-fat">Podíl energie z tuků</label>',
      ...
      '              </select>',
      '              <span class="mb-cm__hint">EFSA uvádí pro dospělé referenční rozmezí 20–35 % energie.</span>',
      '            </div>',
      '            <div class="mb-cm__field mb-cm__field--full">',
      '              <label class="mb-cm__label" for="mb-cm-meals">Počet jídel pro orientační rozdělení</label>',
      '              <select class="mb-cm__select" id="mb-cm-meals">',
      '                <option value="3">3 jídla</option>',
      '                <option value="4" selected>4 jídla</option>',
      '                <option value="5">5 jídel</option>',
      '                <option value="6">6 jídel</option>',
      '              </select>',
      '            </div>',
      '          </div>',
      '        </div>',
      ...
      '        <button class="mb-cm__button mb-cm__button--primary" type="submit">Vypočítat příjem a makra</button>',
      '        <button class="mb-cm__button mb-cm__button--secondary" type="button" id="mb-cm-reset">Vymazat údaje</button>',
      '      </div>',
      ...
      '        <div>',
      '          <p class="mb-cm__result-label">Orientační denní energetický příjem podle cíle</p>',
      '          <div class="mb-cm__score-row"><p class="mb-cm__score" id="mb-cm-target">0</p><span class="mb-cm__score-unit">kcal / den</span></div>',
      '        </div>',
      '        <span class="mb-cm__method" id="mb-cm-method">Udržení hmotnosti</span>',
      '      </div>',
      '      <p class="mb-cm__summary" id="mb-cm-summary"></p>',
      '      <div class="mb-cm__metrics">',
      '        <div class="mb-cm__metric"><p class="mb-cm__metric-label">Klidový výdej (REE)</p><p class="mb-cm__metric-value" id="mb-cm-bmr">0 kcal</p></div>',
      '        <div class="mb-cm__metric"><p class="mb-cm__metric-label">Odhad TDEE</p><p class="mb-cm__metric-value" id="mb-cm-tdee">0 kcal</p></div>',
      '        <div class="mb-cm__metric"><p class="mb-cm__metric-label">Rozdíl oproti TDEE</p><p class="mb-cm__metric-value" id="mb-cm-difference">0 kcal</p></div>',
      '        <div class="mb-cm__metric"><p class="mb-cm__metric-label">Energie v kJ</p><p class="mb-cm__metric-value" id="mb-cm-kj">0 kJ</p></div>',
      '      </div>',
      '      <h3 class="mb-cm__section-title">Orientační rozdělení makroživin</h3>',
      '      <div class="mb-cm__macros">',
      '        <div class="mb-cm__macro"><div class="mb-cm__macro-top"><span class="mb-cm__dot mb-cm__dot--protein"></span><p class="mb-cm__macro-name">Bílkoviny</p></div><p class="mb-cm__macro-value" id="mb-cm-protein-value">0 g</p><p class="mb-cm__macro-meta" id="mb-cm-protein-meta"></p></div>',
      ...
      '      </div>',
      '      <div class="mb-cm__bar" aria-label="Podíl energie z bílkovin, tuků a sacharidů">',
      ...
      '      <p class="mb-cm__disclaimer"><strong>Výsledek je orientační.</strong> Skutečnou potřebu ovlivňuje tělesné složení, spontánní pohyb, intenzita tréninku, zdravotní stav, spánek, léčba i dlouhodobá adaptace organismu.</p>',
      '    </section>',
      '    <p class="mb-cm__privacy">Výpočet probíhá pouze ve vašem prohlížeči. Zadané údaje tento skript nikam neodesílá ani neukládá.</p>',
      '  </div>',
      '</section>'
    ].join('');
    ...
    var rateOptions = {
      loss: [
        { value: '-0.10', label: 'Mírný deficit −10 %' },
        { value: '-0.15', label: 'Střední deficit −15 %', selected: true },
      ...
      gain: [
        { value: '0.05', label: 'Mírný nadbytek +5 %', selected: true },
        { value: '0.10', label: 'Střední nadbytek +10 %' },
        { value: '0.15', label: 'Vyšší nadbytek +15 %' }
      ]
    };
    ...
    function getGoalLabel(goal, delta) {
      if (goal === 'loss') {
        return 'Snížení hmotnosti ' + Math.abs(Math.round(delta * 100)) + ' % pod TDEE';
      }
      if (goal === 'gain') {
        return 'Zvýšení hmotnosti ' + Math.round(delta * 100) + ' % nad TDEE';
      }
    ...
    function calculate(event) {
      ...
      if (!Number.isFinite(age) || age < 18 || age > 100) {
        errors.push('Zadejte věk v rozmezí 18 až 100 let.');
      }
      if (!Number.isFinite(height) || height < 100 || height > 250) {
        errors.push('Zadejte výšku v rozmezí 100 až 250 cm.');
      }
      if (!Number.isFinite(weight) || weight < 30 || weight > 400) {
        errors.push('Zadejte hmotnost v rozmezí 30 až 400 kg.');
      }
      if (!Number.isFinite(pal) || pal < 1.2 || pal > 2.5) {
        errors.push('Vyberte platnou úroveň aktivity.');
      }
      if (!Number.isFinite(delta) || delta < -0.5 || delta > 0.5) {
        errors.push('Vyberte platnou modelovou úpravu příjmu.');
      }
      if (!Number.isFinite(proteinPerKg) || proteinPerKg < 0.5 || proteinPerKg > 3.0) {
        errors.push('Vyberte platné množství bílkovin.');
      }
      if (!Number.isFinite(fatPercent) || fatPercent < 15 || fatPercent > 45) {
        errors.push('Vyberte platný podíl tuků.');
      }
      ...
      if (carbsCalories <= 0) {
        showError(['Zvolená kombinace energetického příjmu, bílkovin a tuků nezanechává prostor pro sacharidy. Snižte množství bílkovin nebo tuků, případně zvolte vyšší energetický příjem.']);
        return;
      }
      ...
      if (target < ree) {
        warnings.push('Vypočtený příjem je nižší než odhadovaný klidový výdej. BMR není pevná bezpečnostní hranice, ale takové nastavení je vhodné posuzovat individuálně s odborníkem.');
      }
      if (carbsPercent < 45 || carbsPercent > 60) {
        warnings.push('Sacharidy tvoří přibližně ' + formatNumber(carbsPercent, 0) + ' % energie. To je mimo referenční rozmezí EFSA 45–60 %; výsledek vznikl z vámi zvoleného množství bílkovin a tuků.');
      }
      if (bmi >= 30 && proteinPerKg > 1.2) {
        warnings.push('Při vyšším BMI může výpočet bílkovin z aktuální hmotnosti poskytovat vysokou hodnotu. Vhodný základ pro výpočet je možné individualizovat podle tělesného složení a zdravotního stavu.');
      }
      if (carbsGrams < 50) {
        warnings.push('Výsledné množství sacharidů je velmi nízké. Takové rozdělení není vhodné automaticky používat bez posouzení celého jídelníčku a zdravotního stavu.');
      }
      ...
      root.querySelector('#mb-cm-summary').textContent = goal === 'maintain'
        ? 'Modelový příjem odpovídá vypočtenému TDEE. Skutečný udržovací příjem se může od odhadu lišit a ověřuje se podle dlouhodobého vývoje hmotnosti a dalších ukazatelů.'
        : 'Modelový příjem je nastaven o ' + Math.abs(Math.round(delta * 100)) + ' % ' + (delta < 0 ? 'níže' : 'výše') + ' než vypočtené TDEE. Výsledek nezaručuje konkrétní rychlost změny hmotnosti.';
      ...
      root.querySelector('#mb-cm-meal').innerHTML = '<strong>Orientační průměr na jedno z ' + meals + ' jídel:</strong> ' +
        formatInteger(target / meals) + ' kcal, ' +
        formatNumber(proteinGrams / meals, 1) + ' g bílkovin, ' +
        formatNumber(fatGrams / meals, 1) + ' g tuků a ' +
        formatNumber(carbsGrams / meals, 1) + ' g sacharidů. Reálná jídla nemusí být rozdělena rovnoměrně.';

      root.querySelector('#mb-cm-formula').textContent = 'Výpočet: REE ' + formatInteger(ree) + ' kcal × PAL ' + formatNumber(pal, 1) + ' = TDEE ' + formatInteger(tdee) + ' kcal; cíl ' + (delta >= 0 ? '+' : '') + formatNumber(delta * 100, 0) + ' % = ' + formatInteger(target) + ' kcal. Bílkoviny mají 4 kcal/g, tuky 9 kcal/g a sacharidy 4 kcal/g.';

      root.querySelector('#mb-cm-warnings').innerHTML = warnings.map(function (warning) {
        return '<p class="mb-cm__warning"><strong>Upozornění:</strong> ' + escapeHtml(warning) + '</p>';
      }).join('');
